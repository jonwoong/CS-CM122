...
if __name__ == "__main__":
    ...
    candidate_snps = defaultdict(list) # stores all possible snps to filter later
    for read in reads:
        match_positions, possible_snps = align(read,reference,hash_table)
        if len(possible_snps) > 0:
            for snp in possible_snps:
                snp_position = snp[0]
                read_base = snp[1]
                candidate_snps[snp_position].append(read_base)
    ...
    # count snps for each position
    snp_counts = defaultdict(list)
    for snp_position, snp_list in candidate_snps.items():
        for read_base in set(snp_list):
            snp_count = candidate_snps[snp_position].count(read_base)
            if snp_count > 100: # keep snps that appear more than 60 times
                snp_counts[snp_position].append((read_base,snp_count))
    #print snp_counts
    ...
    with open(exons_fn) as exons_file:
        ...
        for line in exons_file:
            ...
            # reduce read_count based on how many snps we found in each snp position
            
            for snp_position, snp_list in snp_counts.items():
                if snp_position in range(start,end):
                    for read_base, snp_count in snp_list:
                        if reference[snp_position] != read_base:
                            read_count -= snp_count/coverage
                            if read_count < 0:
                                read_count = 0
